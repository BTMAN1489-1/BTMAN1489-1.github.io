...
function t_throttle(fn, threshhold, scope) {
    ...
    function() {
        var context = scope || this
          , now = +new Date
          , args = arguments;
        last && now < last + threshhold ? (clearTimeout(deferTimer),
...
function t_onFuncLoad(funcName, okFunc, time) {
    if ("function" == typeof window[funcName])
        ...
        setTimeout((function checkFuncExist() {
            ...
            "function" != typeof window[funcName] ? ("complete" === document.readyState && currentTime - startTime > 5e3 && "function" != typeof window[funcName] && callbackError(),
...
function t_scrollBarWidthCompensator__setObject() {
    ...
    obj.isInited = !1,
    obj.scrollBarWidth = Math.abs(window.innerWidth - document.documentElement.clientWidth),
    obj.delay = 0,
...
function t_scrollBarWidthCompensator__init() {
    ...
    var obj = window.scrollBarWidthCompensator;
    if (obj.scrollBarWidth = Math.abs(window.innerWidth - document.documentElement.clientWidth),
    obj.cancelTimeout && (window.clearTimeout(obj.cancelTimeout),
    obj.cancelTimeout = null),
    !obj.isInited && obj.scrollBarWidth) {
        ...
        bodyInlinePadding && document.body.setAttribute("data-tilda-initial-padding-right", bodyInlinePadding),
        document.body.style.paddingRight = obj.scrollBarWidth + bodyComputedPadding + "px",
        ...
        Array.prototype.forEach.call(obj.fixedElements, (function(entry) {
            ...
            if (document.body.contains(el) && !el.classList.contains("t975") && !el.classList.contains("t975")) {
                ...
                if ("fixed" === position || "absolute" === position) {
                    ...
                    duration.indexOf("ms") + 1 ? (duration = parseInt(duration.replace("ms", ""), 10),
                    transitionDurations.push(duration)) : duration.indexOf("s") + 1 && (duration = 1e3 * parseFloat(duration.replace("s", "")),
...
function t_scrollBarWidthCompensator__cancel() {
    var obj = window.scrollBarWidthCompensator;
    obj && obj.isInited && (obj.isInited = !1,
    obj.delay = 0,
    ...
    Array.prototype.forEach.call(obj.fixedElements, (function(entry) {
        var el = entry.el;
        el.hasAttribute("data-tilda-initial-right") ? (el.style.right = el.getAttribute("data-tilda-initial-right"),
        el.removeAttribute("data-tilda-initial-right")) : el.style.removeProperty("right"),
        el.hasAttribute("data-tilda-initial-width") ? (el.style.width = el.getAttribute("data-tilda-initial-width"),
        el.removeAttribute("data-tilda-initial-width")) : el.style.removeProperty("width")
    }
    )))
}
...
t_onReady((function() {
    var userAgent = window.navigator.userAgent, isInstagram = -1 !== userAgent.indexOf("Instagram"), isFacebook = -1 !== userAgent.indexOf("FBAV"), isYandex = -1 !== userAgent.indexOf("YaSearchBrowser"), isSamsung = -1 !== userAgent.indexOf("SamsungBrowser"), isDuckDuckGo = -1 !== userAgent.indexOf("DuckDuckGo"), isAndroid;
    if (-1 !== userAgent.indexOf("Android") && (isFacebook || isInstagram || isYandex || isSamsung || isDuckDuckGo)) {
        ...
        factor < .98 && document.body.insertAdjacentHTML("beforeend", '<style>.t396 [data-elem-type="text"] .tn-atom {zoom: ' + 100 * factor + "%;}</style>")
    }
...
t_onReady((function() {
    setTimeout((function() {
        ...
        if (document.body && (bodyScrollHeight = Math.max(document.body.scrollHeight, document.body.offsetHeight, document.body.clientHeight, html.offsetHeight)),
        (document.getElementById("tildacopy") || tildaLabel) && tildaLabel.querySelectorAll("div"))
            bodyScrollHeight + 70 > window.innerHeight && tildaLabel && tildaLabel.setAttribute("style", "display: block !important; visibility: visible !important; position: relative !important; width: 100% !important; pointer-events: all !important; opacity: 1 !important; margin: 0 !important; z-index: 1 !important");
        else {
...
t_onReady((function() {
    ...
    if (!window.isMobile && allRecords && "yes" !== allRecords.getAttribute("data-blocks-animationoff") && !1 === window.isSearchBot) {
        ...
        for (var recBlocksNot = Array.prototype.slice.call(recBlocks).filter((function(el) {
            ...
            var rec, recTop = (rec = recBlocksNot[i]).getBoundingClientRect().top + window.pageYOffset, position = window.pageYOffset + window.innerHeight + 300;
            t_addClass(rec, recTop > 1e3 && recTop > position ? "r_hidden" : "r_showed"),
        ...
        if (recBlocksNot.length) {
            function t_blocksFade() {
                for (var i = recBlocksNot.length - 1; i >= 0; i--) {
                    ...
                    rec.getBoundingClientRect().top + window.pageYOffset < (position = rec.offsetHeight <= 100 ? window.pageYOffset + window.innerHeight : window.pageYOffset + window.innerHeight - 100) && (t_removeClass(rec, "r_hidden"),
                    t_addClass(rec, "r_showed"),
                    (recBlocksNot = Array.prototype.slice.call(recBlocksNot)).splice(i, 1))
                }
            }
            ...
            if (recBlocksT400.length > 0)
                var countCompleted = 0
                  ...
                  , timer = setInterval((function() {
                    ...
                    countCompleted === recBlocksT400.length && (t_blocksFade(),
                    clearInterval(timer))
                }
                ), 100);
...
function() {
    ...
    function t_blocksdisplay() {
        var windowWidth = window.isMobile ? document.documentElement.clientWidth : window.innerWidth, recBlocks = document.querySelectorAll(".r[data-screen-max], .r[data-screen-min]"), max, min, display;
        -1 !== navigator.userAgent.indexOf("Instagram") && (windowWidth = screen.width);
        for (var i = 0; i < recBlocks.length; i++) {
            var rec = recBlocks[i];
            if ("yes" === rec.getAttribute("data-connect-with-tab"))
                return;
            ...
            max = parseInt(max),
            (min = parseInt(min)) <= max && (windowWidth <= max && windowWidth > min ? "block" !== display && (rec.style.display = "block") : "none" !== display && (rec.style.display = "none"))
        }
    }
    t_onReady((function() {
        t_setWindowVars(),
...
function() {
    ...
    function t_correctHeight() {
        ...
        for (var i = 0; i < elCarries.length; i++) {
            var element, elementStyle;
            (elementStyle = (element = elCarries[i]).style).height.indexOf("vh") > -1 && (factor = parseInt(elementStyle.height) / 100,
        ...
        for (var i = 0; i < visibleRecBlocks.length; i++)
            for (var rec = visibleRecBlocks[i], blocks = rec.querySelectorAll('div:not([data-auto-correct-mobile-width="false"]):not(.tn-elem):not(.tn-atom):not(.tn-atom__sbs-anim-wrapper):not(.tn-atom__prx-wrapper):not(.tn-atom__videoiframe):not(.tn-atom__sticky-wrapper):not(.t-store__relevants__container):not(.t-slds__items-wrapper):not(.js-product-controls-wrapper):not(.js-product-edition-option):not(.t-product__option-variants)'), j = 0; j < blocks.length; j++) {
                ...
                if (blockWidth > windowWidth) {
                    if ("yes" === block.getAttribute("[data-customstyle]") && "false" === block.parentNode.getAttribute("[data-auto-correct-mobile-width]"))
                        return;
                    ...
                    rec.style.wordBreak = blockWidth - 3 > windowWidth ? "break-word" : ""
                }
            }
    }
        ...
        for (var elements = document.querySelectorAll('.t-text:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-name:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-title:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-descr:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-heading:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-text-impact:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-subtitle:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"]), .t-uptitle:not(.tn-elem):not(.tn-atom):not([data-auto-correct-line-height="false"])'), i = 0; i < elements.length; i++) {
            ...
            if (elementStyle) {
                ...
                if (document.documentElement.clientWidth > breakpoint)
                    newStyle = (newStyle = elementStyle.replace("lineheight", "line-height")).replace("fontsize", "font-size"),
                    element.setAttribute("style", newStyle);
                else {
                    if (-1 === elementStyle.indexOf("font-size"))
                        continue;
                    if (parseInt(getComputedStyle(element).fontSize.replace("px", "")) < 26)
                        continue;
                    newStyle = elementStyle.replace("line-height", "lineheight"),
                    newStyle = isREMValue ? newStyle.replace(/font-size.*px;/gi, "font-size: 1.6rem;") : newStyle.replace("font-size", "fontsize"),
                    element.setAttribute("style", newStyle)
                }
            }
        }
...
t_onReady((function() {
    ...
    document.body.addEventListener("popupHidden", (function() {
        var obj = window.scrollBarWidthCompensator;
        obj && (obj.cancelTimeout && (window.clearTimeout(obj.cancelTimeout),
        obj.cancelTimeout = null),
